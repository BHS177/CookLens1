...
// OpenAI Vision API Configuration (using GPT-4o for image analysis)
...
Répondez UNIQUEMENT au format JSON avec cette structure exacte:
{
  ...
  "reheating": "Conseils de réchauffage"
    ...
    // STRICT validation that all ingredients in the recipe are from the detected list
    const detectedIngredientNames = ingredients.map(ing => ing.name.toLowerCase())
    const recipeIngredients = recipeData.ingredients || []
    
    console.log('🔍 Validating recipe ingredients against detected ingredients...')
    console.log('📋 Detected ingredients:', detectedIngredientNames)
    console.log('📋 Recipe ingredients:', recipeIngredients.map((ing: any) => ing.name))
...
export async function generateRecipeSuggestions(
  ...
  const ingredientNames = request.ingredients.map(ing => ing.name).join(', ')
  
  const prompt = `Tu es un chef professionnel étoilé. Crée 3 suggestions de recettes différentes en utilisant ces ingrédients: ${ingredientNames}.

Préférences utilisateur:
- Cuisine: ${request.preferences.cuisine || 'Française'}
- Régime: ${request.preferences.diet || 'Normal'}
- Allergies: ${request.preferences.allergies?.join(', ') || 'Aucune'}
- Difficulté: ${request.preferences.difficulty || 'Moyen'}

Répondez UNIQUEMENT au format JSON avec cette structure exacte:
[
  {
    "title": "Nom de la recette 1",
  ...
  },
  {
    ...
    console.log('✅ ChatGPT suggestions received:', content)
    
    // Parse JSON response
    const suggestionsData = JSON.parse(content)
    
    // Convert to RecipeSuggestion format
    const suggestions: RecipeSuggestion[] = suggestionsData.map((suggestion: any, index: number) => ({
      ...
      mainIngredients: suggestion.mainIngredients,
      techniques: suggestion.techniques,
      tags: suggestion.tags,
      createdAt: new Date()
    }))
    
    console.log('✅ Recipe suggestions generated successfully:', suggestions.length)
    return suggestions
